...
class lstm_query_usrn_con_et_mf_v26_tes_th(lstm_query_usrn_con_mf_test):
    def __init__(self, args, user_triplet_set, rela_2_index, act_dim, gamma=0.99, hidden_sizes=[512, 256]):
        super().__init__(args, user_triplet_set, rela_2_index, act_dim, gamma, hidden_sizes)
    ...
    def update_path_info_memories(self, up_date_hop):
        ...
        # new_current_step_user = []
        # new_current_step_path = []
        # new_current_step_actions= []

        # for row in up_date_hop:
        #     new_current_step_user.append(self.current_step['user'][row])
        #     new_current_step_path.append(self.current_step['path'][row])
        #     new_current_step_actions.append(self.current_step['actions'][row])

        # self.current_step['user'] = new_current_step_user
        # self.current_step["path"] = new_current_step_path
        # self.current_step["actions"] = new_current_step_actions
    ...
    def generate_act_emb(self, batch_path, batch_curr_actions):
        ...
        # print('b_path = ', b_path)
        # input()
        # # self.args.data_entity_ind_id

        self.current_step["path"] = ['path = ' + ', next = '.join(cn_b_path) for cn_b_path in b_path]
        self.current_step["actions"] = b_action
    ...
    def _record_case_study(self):
        ...
        for ind in range(len(self.current_step['user'])):
            ...
            for ac_pro in self.current_step['actions_pro'][ind]:
                ...
                eva_file.write(ac_pro)
                eva_file.write('\n')
            eva_file.write("next_action = " + self.current_step['next_acts'][ind])
            eva_file.write('\n')
            eva_file.write("*" * 50)
            eva_file.close()
...
class lstm_query_usrn_con_et_mf_v28_test_no_grad_th(lstm_query_usrn_con_mf_test):
    def __init__(self, args, user_triplet_set, rela_2_index, act_dim, gamma=0.99, hidden_sizes=[512, 256]):
        super().__init__(args, user_triplet_set, rela_2_index, act_dim, gamma, hidden_sizes)
        self.scalar = Variable(torch.Tensor([0.5]), requires_grad=True).cuda().to(self.device)
        self.scalar = nn.Parameter(self.scalar)
    ...
    def rn_query_st(self, state, relation_embed_dual, rn_step):
        ...
        for hop in range(self.p_hop):
            ...
            for prob_index in range(len(probs_expanded_to_list)):
                ...
                # sorted(by_hop_query_dict, key = lambda s: s[2])
                # print('by_hop_query_highlight_list = ',by_hop_query_highlight_list)
                # input()
    ...
    def generate_act_emb(self, batch_path, batch_curr_actions):
        ...
        # print('self.current_step[actions] = ', self.current_step["actions"] )
        # print('self.current_step[path] = ', self.current_step["path"] )
