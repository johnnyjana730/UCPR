...
def get_user_triplet_set(kg, et_idx2ty, user_list, p_hop, n_memory):
    ...
    def _batch_get_actions(self, batch_path, done):
        def _get_actions(index, path, done):
            ...
            for r in relations_nodes:
                ...
                next_node_set = [[self.et_idx2ty[n_id],n_id] for n_id in next_node_ids if self.et_idx2ty[n_id] != USER]
                next_node_ids = [n_set[1] for n_set in next_node_set if (n_set[0], n_set[1]) not in visited_nodes]  # filter
