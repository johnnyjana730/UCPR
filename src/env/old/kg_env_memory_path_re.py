    ...
    for h in range(max(1,p_hop)):
        ...
        for entity_type, entity in tails_of_last_hop:
            ...
            for k_, v_set in g_kg(entity_type,entity).items():
                ...
                if k_ == 'mentions' or k_ == 'described_as' or \
                    k_ == 'also_viewed': continue
                # print('k_, v_set = ', k_, v_set)
...
class BatchKGEnvironment(object):
    def __init__(self, args, dataset_str, max_acts, max_path_len=3, state_history=1):
        ...
        dataset = load_dataset_core(args, dataset_str)
        ...
        self.record_path_info_path = '{}/rd_{}_ic_{}_ac_{}_uc_{}_info_all.pickle'.format(SAVE_MODEL_DIR_debug[args.dataset], self.args.dataset, 
                    self.args.item_core, self.args.att_core, self.args.user_core)
        if  os.path.isfile(self.record_path_info_path):
            with open(self.record_path_info_path, 'rb') as fp:
                self.record_path_info = pickle.load(fp)
            self.args.logger.info('load = ' + self.record_path_info_path)
            # print('self.record_path_info = ', self.record_path_info)
        else:
            self.record_path_info = {}

        print('_batch_get_actions_all ')
        # input()
        self._batch_get_actions_all()
        # print('after _batch_get_actions_all ')
        # input()
    ...
    def _batch_get_actions(self, batch_path, done):
        def _get_actions(index, path, done):
            def queru_next_possible_action(curr_node_type, curr_node_id):
                ...
                for r in relations_nodes:
                    ...
                    if next_node_type == USER:
                        next_node_ids = [id_ for id_ in next_node_ids if id_ in self.args.kg_user_filter]  
            ...
            visited_nodes = set([(v[1], v[2]) for v in path])            
            candidate_acts = [(r, n_id) for r, n_id in candidate_acts if (KG_RELATION[curr_node_type][r], n_id) not in visited_nodes]
    ...
    def _batch_get_actions_all(self):
        def queru_next_possible_action(curr_node_type, curr_node_id):
            relations_nodes = self.kg(curr_node_type, curr_node_id)
            candidate_acts = []  # list of tuples of (relation, node_type, node_id)
            candidate_node_types = []
        ...
        sus, fail = 0, 0
        for etype1, v_ in self.kg.G.items():
            for eid1, v_ in tqdm(self.kg.G[etype1].items()):
                curr_node_type, curr_node_id = etype1, eid1
                cur_st_key = 'type_'+ str(curr_node_type) + 'id_' + str(curr_node_id)
                if cur_st_key not in self.record_path_info:
                    candidate_acts = queru_next_possible_action(curr_node_type,curr_node_id)
                    self.record_path_info[cur_st_key] = candidate_acts
        ...
        # print('sus = ', sus, len(self.record_path_info), 'fail = ', fail)
        # input()

        self.args.logger.info('save = ' + self.record_path_info_path)
        with open(self.record_path_info_path, 'wb') as fp:
            pickle.dump(self.record_path_info, fp)
