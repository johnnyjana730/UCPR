...
def predict_paths(policy_file, path_file, args):
    ...
    trn_labels = load_labels_core(args,args.dataset, 'train')

    test_labels = load_labels_core(args,args.dataset, 'test')
...
def evaluate_paths(top_k, path_file, eva_file, train_labels, test_labels):
    ...
    eva_file.write('Epoch = ' + str(args.eva_epochs) + ', top_k = ' + str(top_k) + '\n')
...
def test(args):
    if args.pretrained_st_epoch < args.eva_epochs:
        ...
        eva_file = args.log_dir + '/policy_paths_evaluation_newg_top_k_test.txt'

        train_labels = load_labels_core(args,args.dataset, 'train')
        test_labels = load_labels_core(args,args.dataset, 'test')
...
def setup_index_to_name(args):
    datadir = DATASET_DIR[args.dataset]
    ...
    with open(f"{datadir}/new_meta_Books_dict.pickle",'rb') as f:
        new_meta_Books_dict = pickle.load(f)

    rating_file = datadir + '/ab2fb'
    ...
    rating_file = datadir + '/entity_list'
    fp = open(rating_file + '.txt', "r")
    index_2_freebase = {}
    index_2_entity = {}
    for line in iter(fp):
        ...
        if line_tr[-1].isdigit():
            line_tr[-1] = str(int(line_tr[-1]) + n_user)
    ...
    rating_file = datadir + '/relation_list'
    fp = open(rating_file + '.txt', "r")
    rela_2_name = {}
    for line in iter(fp):
    ...
    args.index_2_entity = index_2_entity
    args.rela_2_name = rela_2_name
...
if __name__ == '__main__':
    ...
    # args.name = args.name + 'sv_g'
    
    if args.h0_embbed == True:
        log_dir_fodder = f'lr_{args.lr}_bs_{args.batch_size}_sb_{args.sub_batch_size}_ma_{args.max_acts}_p0{args.p_hop}_{args.name}_g_aiu_{args.att_core}_{args.item_core}_{args.user_core}'
    else:
        log_dir_fodder = f'lr_{args.lr}_bs_{args.batch_size}_sb_{args.sub_batch_size}_ma_{args.max_acts}_p1{args.p_hop}_{args.name}_g_aiu_{args.att_core}_{args.item_core}_{args.user_core}'
    ...
    args.log_dir = '{}/{}/{}'.format(EVALUATION_debug_1[args.dataset], args.use_men, log_dir_fodder)
    if not os.path.isdir(args.log_dir):
        os.makedirs(args.log_dir)

    args.cast_st_dir = '{}/{}/{}'.format(CASE_ST[args.dataset], args.use_men + '_az_et', log_dir_fodder)
