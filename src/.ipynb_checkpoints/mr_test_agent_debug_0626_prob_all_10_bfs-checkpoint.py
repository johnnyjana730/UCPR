...
def batch_beam_search(args, env, model, uids, device, topk=[25, 5, 1], topk_list= [1,25,125,125]):
    ...
    for hop in range(3):
        ...
        actmask_tensor = actmask_tensor.detach().cpu()


        probs =  np.array([[1.0] * topk[hop] for _ in range(actmask_tensor.shape[0])])
        # print('probs = ', probs.shape)
        # input()
        probs = torch.from_numpy(probs)

        # print('actmask_tensor = ', actmask_tensor.shape)
        probs = probs + actmask_tensor  # In order to differ from masked actions
        ...
        # torch.arange(0, topk[hop])
        # np.array([for _ in range(actmask_tensor.shape[0])])
        topk_idxs = topk_idxs.numpy()
...
def evaluate_paths_prof(args, top_k, path_file, eva_file, train_labels, test_labels):
    ...
    eva_file.write('Epoch = ' + str(args.eva_epochs) + ', top_k = ' + str(top_k) + 'bfs test 1, 10, 150, 150' + '\n')
...
def test(args):
    if args.pretrained_st_epoch < args.eva_epochs:
        ...
        if args.run_eval:
            ...
            eva_file = args.log_dir + '/eva_top_k_prob_132651_set.txt'

            for top_k in [132651]:
                evaluate_paths_prof(args, top_k, path_file, eva_file, train_labels, test_labels)
...
if __name__ == '__main__':
    ...
    args.topk = [51, 51, 51]
    args.topk_list = [1, 50, 2601, 132651]
