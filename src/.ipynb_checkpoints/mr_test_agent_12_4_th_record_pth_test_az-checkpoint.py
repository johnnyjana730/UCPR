...
from utils import *
from model_refine.get_model_az_test import *
from parser import parse_args
from parameter_setting import parameter_env

from sklearn.feature_extraction.text import TfidfVectorizer
# import Levenshtein
from data_utils import AmazonDataset_userth
...
def batch_beam_search(args, env, model, uids, device, topk=[25, 5, 1], topk_list= [1,25,125,125]):
    ...
    for hop in range(3):
        ...
        prob_tmp = probs.tolist()
        for act_, prob_ in zip(model.current_step["actions"], prob_tmp):
            new_action_pro_set = []
            for con_act_, con_prob_ in zip(act_, prob_):
                prob_ = str(round(con_prob_, 5))
                action_prob_list = ' prob = '.join([con_act_, prob_])
                new_action_pro_set.append(action_prob_list)
            model.current_step['actions_pro'].append(new_action_pro_set)

        topk_probs, topk_idxs = torch.topk(probs, topk[hop], dim=1)  # LongTensor of [bs, k]
        # print('str(topk_idxs.tolist()) = ', str(topk_idxs.tolist()))
        # input()
        model.current_step['next_acts'] = str(topk_idxs.tolist())
        model._record_case_study_az()
...
def predict_paths(policy_file, path_file, args):
    ...
    while start_idx < len(test_uids):
        ...
        # print('start_idx, times = ',start_idx , times)
        pbar.update(batch_size)
    predicts = {'paths': all_paths, 'probs': all_probs}
    pickle.dump(predicts, open(path_file, 'wb'))


def evaluate_paths_prob(top_k, path_file, eva_file, train_labels, test_labels):
    ...
    for uid in best_pred_paths:
        ...
        top_k_pids = [p for _, _, p in sorted_path[:top_k]]  # from largest to smallest
        
        top_k_pids_path = []

        for pth in top_k_pids:
            top_k_pids_tmp = [[pth_seg[0], pth_seg[1], data_entity_ind_id[pth_seg[1]][pth_seg[2]]] \
                if pth_seg[1] in data_entity_ind_id and pth_seg[2] in data_entity_ind_id[pth_seg[1]] else pth_seg for pth_seg in pth]
            
            path_info = ''
            for seg_pt in top_k_pids_tmp:
                if seg_pt[2] in asin2title:
                    seg_pt[2] = str(asin2title[seg_pt[2]][:30])
                path_info += seg_pt[0][:4] + '_' +  seg_pt[1][:4] + '_' + seg_pt[2] + ' -> ' 
            path_info += '\n'
            top_k_pids_path.append(path_info)

        pred_labels[uid] = top_k_pids_path  # change order to from smallest to largest!

        eva_file.write('uid = ' + str(uid) + '_' + str(len(train_labels[uid])) + '\n')

        for wit_path in top_k_pids_path:
            print('wit_path = ', wit_path)
            eva_file.write(wit_path.encode('utf-8').decode('utf8'))
        eva_file.write('\n')
...
def test(args):
    if args.pretrained_st_epoch < args.eva_epochs:
        ...
        path_file = args.save_model_dir + '/policy_paths_epoch{}_{}.pkl'.format(args.eva_epochs, args.topk_string)
        ...
        if args.run_eval:
            ...
            for top_k in [5, 10, 20]:
                eva_file = args.log_dir + '/eva_path_' + str(top_k) + '_epo_' + str(args.eva_epochs) + '_' + args.topk_string + '.txt'
                evaluate_paths_prob(top_k, path_file, eva_file, train_labels, test_labels)
            ...
            eva_file = args.log_dir + '/exp_eva_score_top_k_all.txt'
            
            for top_k in [5]:
                evaluate_paths_score(top_k, path_file, eva_file, train_labels, test_labels)
...
if __name__ == '__main__':
    ...
    if args.reward_rh == 'hybrid':
        args.log_dir = '{}/{}/{}'.format(EVALUATION_debug_1[args.dataset], args.use_men + '_rh_hb', log_dir_fodder)
        if not os.path.isdir(args.log_dir):
            os.makedirs(args.log_dir)
    elif args.reward_rh == 'pgpr':
        args.log_dir = '{}/{}/{}'.format(EVALUATION_debug_1[args.dataset], args.use_men + '_rh_pgpr', log_dir_fodder)
        if not os.path.isdir(args.log_dir):
            os.makedirs(args.log_dir)
    else:
        args.log_dir = '{}/{}/{}'.format(EVALUATION_debug_1[args.dataset], args.use_men +'_rh_no', log_dir_fodder)
        if not os.path.isdir(args.log_dir):
            os.makedirs(args.log_dir)
    ...
    dataset_test = AmazonDataset_userth(args, DATASET_DIR[args.dataset], set_name = 'test')

    save_dataset_core_th_test(args, args.dataset, dataset_test)

    dataset_test = load_dataset_core_th_test(args, args.dataset)

    dataset = load_dataset_core_th(args, args.dataset)

    data_entity_ind_id = {}

    # for key_id in [USER, WORD, PRODUCT, BRAND, CATEGORY, RPRODUCT]:
    #     data_entity_ind_id[key_id] = {}
    #     for w_i, element in enumerate(dataset[key_id].vocab):
    #         data_entity_ind_id[key_id][w_i] = element


    data_entity_ind_id[USER] = {}
    for w_i, element in enumerate(dataset.user.vocab):
        data_entity_ind_id[USER][w_i] = element

    data_entity_ind_id[WORD] = {}
    for w_i, element in enumerate(dataset.word.vocab):
        data_entity_ind_id[WORD][w_i] = element

    data_entity_ind_id[PRODUCT] = {}
    for w_i, element in enumerate(dataset.product.vocab):
        data_entity_ind_id[PRODUCT][w_i] = element

    data_entity_ind_id[BRAND] = {}
    for w_i, element in enumerate(dataset.brand.vocab):
        data_entity_ind_id[BRAND][w_i] = element

    data_entity_ind_id[CATEGORY] = {}
    for w_i, element in enumerate(dataset.category.vocab):
        data_entity_ind_id[CATEGORY][w_i] = element

    data_entity_ind_id[RPRODUCT] = {}
    for w_i, element in enumerate(dataset.related_product.vocab):
        data_entity_ind_id[RPRODUCT][w_i] = element

    args.data_entity_ind_id = data_entity_ind_id

    import gzip
    
    METADATA = {'beauty_core': 'meta_Beauty.json.gz',
             'cloth_core': 'meta_Clothing_Shoes_and_Jewelry.json.gz',
             'cell_core': 'meta_Cell_Phones_and_Accessories.json.gz'}

    path_meta_data = "/home/billhuang/Paper_code/PGPR/"
    ...
    meta = parse(path_meta_data + METADATA[args.dataset])
        
    asin2title = {}
    while True:
        try:
            x = next(meta)
            if 'asin' not in x or 'title' not in x:
                continue
            asin2title[x['asin']] = x['title'][:30]
        except StopIteration:
            break

    args.asin2title = asin2title

    # total_corpus = []
    # user_review_dic = {}
    # for rid, data in enumerate(dataset.review.data):
    #     uid, pid, review = data
    #     review_text = [word_vacab[w] for w in review]
    #     total_corpus.append(' '.join(review_text))

    # for rid, data in enumerate(dataset_test.review.data):
    #     uid, pid, review = data
    #     review_text = [word_vacab[w] for w in review]
    #     total_corpus.append(' '.join(review_text))
    ...
    # tfidf_vectorizer_vectors = tfidf_vectorizer.fit_transform(total_corpus)
    ...
    # tfidf_value = dict(zip(tfidf_vectorizer.get_feature_names(), means.tolist()[0]))

    # for rid, data in enumerate(dataset.review.data):
    #     uid, pid, review = data
    #     if uid not in user_review_dic:
    #         user_review_dic[uid] = {}
    #     if pid not in user_review_dic[uid]:
    #         user_review_dic[uid][pid] = []
    #     review_text = [word_vacab[w] for w in review if word_vacab[w] in tfidf_value and tfidf_value[word_vacab[w]] > 0.1]
    #     for w in review_text:
    #         user_review_dic[uid][pid].append(w)
