...
def evaluate(topk_matches, test_user_products, no_skip_user):
    ...
    print('cum_k == 0 ',  cum_k)
    return avg_precision, avg_recall, avg_ndcg, avg_hit, [ndcgs, recalls, precisions, hits], invalid_users, cum_k
...
def write_file(path, list_tmp):
    t_test_result = ','.join([str(k) for k in list_tmp])
    path_file = open(path, "w")
    path_file.write(t_test_result)
    path_file.close()


def evaluate_paths_prob(args, top_k, path_file, eva_file, train_labels, test_labels):
    ...
    avg_precision, avg_recall, avg_ndcg, avg_hit, [ndcgs, recalls, precisions, hits], invalid_users, cum_k = evaluate(pred_labels, test_labels, no_skip_user)
    ...
    ndcg_test_file = args.log_dir + '/test_prob_20_ndcg_set_'+ str(args.eva_epochs) +'.txt'
    write_file(ndcg_test_file, ndcgs)

    recall_test_file = args.log_dir + '/test_prob_20_recall_set_'+ str(args.eva_epochs) +'.txt'
    write_file(recall_test_file, recalls)

    precisions_test_file = args.log_dir + '/test_prob_20_precisions_set_'+ str(args.eva_epochs) +'.txt'
    write_file(precisions_test_file, precisions)

    hits_test_file = args.log_dir + '/test_prob_20_hits_set_'+ str(args.eva_epochs) +'.txt'
    write_file(hits_test_file, hits)

def evaluate_paths_score(args, top_k, path_file, eva_file, train_labels, test_labels):
    embeds = load_embed(args.dataset)
    user_embeds = embeds[USER]
    purchase_embeds = embeds[PURCHASE][0]
    product_embeds = embeds[PRODUCT]
    ...
    eva_file.write('Epoch = ' + str(args.eva_epochs) + ', top_k = ' + str(top_k) + 'no skip sort_by score' + 'topk = 1, 10, 150, 150' + '\n')
    eva_result = 'NDCG={:.3f} |  Recall={:.3f} | HR={:.3f} | Precision={:.3f} | Invalid users={} | cum_k={}'.format(
            avg_ndcg, avg_recall, avg_hit, avg_precision, len(invalid_users), cum_k)
    eva_file.write(eva_result)
    eva_file.write('\n')
...
def test(args):
    if args.pretrained_st_epoch < args.eva_epochs:
        ...
        path_file = args.save_model_dir + '/policy_paths_epoch{}.pkl'.format(args.eva_epochs)
        ...
        if args.run_eval:
            ...
            eva_file = args.log_dir + '/eva_noskip_top_k_20_set.txt'

            for top_k in [20]:
                evaluate_paths_prob(args, top_k, path_file, eva_file, train_labels, test_labels)
            ...
            eva_file = args.log_dir + '/eva_score_noskip_top_k_20_set.txt'
            
            for top_k in [20]:
                evaluate_paths_score(args, top_k, path_file, eva_file, train_labels, test_labels)
