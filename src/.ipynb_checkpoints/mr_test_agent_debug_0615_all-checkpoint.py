...
def predict_paths(policy_file, path_file, args):
    ...
    while start_idx < len(test_uids):
        ...
        try:
            ...
            fail_batch = f"'batch_uids = ', {fail_uids}, error, {str(start_idx)}, {str(end_idx)}"
...
def test(args):
    if args.pretrained_st_epoch < args.eva_epochs:
        ...
        eva_file = args.log_dir + '/policy_paths_evaluation_newg_top_k_all.txt'
...
if __name__ == '__main__':
    ...
    if args.h0_embbed == True:
        log_dir_fodder = f"lr_{args.lr}_bs_{args.batch_size}_sb_{args.sub_batch_size}_ma_{args.max_acts}_p0{args.p_hop}_{args.name}_g_aiu_{args.att_core}_{args.item_core}_{args.user_core}_qy_uam_{args.user_query_threshold}_{args.query_threshold}_{args.query_threshold_maximum}"
    else:
        log_dir_fodder = f'lr_{args.lr}_bs_{args.batch_size}_sb_{args.sub_batch_size}_ma_{args.max_acts}_p1{args.p_hop}_{args.name}_g_aiu_{args.att_core}_{args.item_core}_{args.user_core}_qy_uam_{args.user_query_threshold}_{args.query_threshold}_{args.query_threshold_maximum}'
